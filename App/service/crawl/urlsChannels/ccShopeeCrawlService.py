...
class ccShopeeCrawlService(object):
    ...
    def saveHtmlResult(self, url):
        ...
        itemid = re_url.group(2)
        shopid = re_url.group(1)
        ...
        for useType in '01122':
            ...
            header['TARGETURL'] = shopeurl.format(netloc, itemid, shopid)
            ...
            try:
                ...
                try:
                    ...
                    del data2['sales31']  # '剔除历史销量'
                    del data2['base']['sales_bag']  # '剔除销量包'
                    commonElasticSearchClass.updataDataByIndexKey(index='shopee_info_{}'.format(site), id=md5str,
                                                                  data=data2, elasticsearchType=1)
                except elasticsearch.exceptions.NotFoundError:
                    print('带空插入')
                    commonElasticSearchClass.insertDataByIndexKey(index='shopee_info_{}'.format(site), id=md5str,
                                                                  data=data, elasticsearchType=1)
                commonRedisClass.insertDataByIndexKey(redisKeyName='shopee_info_{}'.format(site), redisStr=md5str)
                self.relayServiceClass.delUrlsCrawlerSuccessByUrl(url=url)
                return
